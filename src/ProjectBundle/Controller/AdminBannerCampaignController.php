...
use ProjectBundle\Form\Type\AdminBannerCampaignType;
...
class AdminBannerCampaignController extends Controller
{
	const ROUTER_INDEX = 'admin_banner_campaign';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  	const ROUTER_PREFIX = 'banner_campaign';
	const ROUTER_CONTROLLER = 'AdminBannerCampaign';

	protected function getQuerySearchData($arr_query_data,$locale)
	{
		$repository = $this->getDoctrine()->getRepository(BannerCampaign::class);
	...
	*/
	public function newAction(Request $request)
	{
		...
		$data = new BannerCampaign();
		$form = $this->createForm(AdminBannerCampaignType::class, $data);
  ...
  */
  public function editAction(Request $request)
  {
    $util = $this->container->get('utilities');
		$util->setCkAuthorized();
		$data = $this->getDoctrine()->getRepository(BannerCampaign::class)->find($request->get('id'));
  ...
  */
  public function updateAction(Request $request)
  {
    ...
    $em = $this->getDoctrine()->getManager();
		...
		$data = $em->getRepository(BannerCampaign::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
    ...
    if ($form->isSubmitted() && $form->isValid()) {
			...
			if($request->get('removefileimage_admin_banner_campaign_translations_en_image')==1){
				$data->translate('en')->setImage(null);
			}

			if($request->get('removefileimage_admin_banner_campaign_translations_en_imageMobile')==1){
				$data->translate('en')->setImageMobile(null);
			}

			if($request->get('removefileimage_admin_banner_campaign_translations_th_image')==1){
				$data->translate('th')->setImage(null);
			}

			if($request->get('removefileimage_admin_banner_campaign_translations_th_imageMobile')==1){
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(BannerCampaign::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
				    $em->flush();
