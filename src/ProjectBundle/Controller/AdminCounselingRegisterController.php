...
class AdminCounselingRegisterController extends Controller
{
	const ROUTER_INDEX = 'admin_counseling_register';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  	const ROUTER_PREFIX = 'counseling_register';
	const ROUTER_CONTROLLER = 'AdminCounselingRegister';

	protected function getQuerySearchData($arr_query_data)
	{
		$repository = $this->getDoctrine()->getRepository(CounselingRegister::class);
	...
	*/
	public function viewAction(Request $request)
	{
    	...
    	$data = $em->getRepository(CounselingRegister::class)->find($request->get('id'));
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(CounselingRegister::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
				    $em->flush();
	...
	*/
	public function _getunreadAction(Request $request)
	{
    	$util = $this->container->get('utilities');
		$em = $this->getDoctrine()->getManager();
    	$data = $em->getRepository(CounselingRegister::class)->countBy(['status'=>4]);
		if($data){
