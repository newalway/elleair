...
use ProjectBundle\Form\Type\AdminJobLocationType;
...
{
    const ROUTER_INDEX = 'admin_job_location';
    const ROUTER_ADD = self::ROUTER_INDEX.'_new';
    const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
    const ROUTER_CONTROLLER = 'AdminJobLocation';

    protected function getQuerySearchData($arr_query_data)
    {
        $repository = $this->getDoctrine()->getRepository(JobLocation::class);
    ...
    */
    public function newAction(Request $request)
    {
        ...
        $data = new JobLocation();
        $form = $this->createForm(AdminJobLocationType::class, $data);
    ...
    */
    public function editAction(Request $request)
    {
        ...
        $data = $this->getDoctrine()->getRepository(JobLocation::class)->find($request->get('id'));
    ...
    */
    public function deleteAction(Request $request)
    {
        ...
        $data = $em->getRepository(JobLocation::class)->find($request->get('id'));
        if (!$data) { throw $this->createNotFoundException('No data found'); }
    ...
    public function group_deleteAction(Request $request)
    {
        ...
        if ($data_ids) {
            ...
            foreach ($data_ids as $data_id) {
                $data = $em->getRepository(JobLocation::class)->find($data_id);
                if ($data) {
                    try {
                        $em->remove($data);
                    $em->flush();
